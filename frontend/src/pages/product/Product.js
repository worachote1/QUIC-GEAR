...
const Product = () => {
    ...
    useEffect(() => {
        ...
        if (searchQuery !== null) {
            ...
            filteredByBrand = filteredByBrand.filter((item) => {
                ...
                const typeSubtype = `${item.type.toLowerCase().replace('&', 'and')}/${item.subType.toLowerCase().replace('&', 'and')}`;
    ...
    }, [selectedBrands, selectedRGB, selectedWireless, searchQuery, selectedFilter, minPrice, maxPrice, allProducts]);
    ...
    return (
        ...
        <div className="flex">
            ...
            <div className={`fixed right-0 w-72 bg-[#ebebeb] text-black z-50 h-screen pt-5 duration-300 overflow-scroll  ${open ? "opacity-100" : "opacity-0 invisible"
                }`}
            >
